...
let print_instruction i = (match i  with
  | Halt -> "Halt"
  | Skip -> "Skip"
  | Label l -> String.concat "" ["THIS SHOULD NOT BE HERE - Label "; string_of_int l]
  | PushS x -> (String.concat "" ["PushS "; string_of_int x])
  | PopS -> "PopS"
  | Add -> "Add"
  | Sub -> "Sub"
  | Div -> "Div"
  ...
  | Eq -> "Eq"
  | Output -> "Output"
  | NonZero x -> String.concat "" ["NonZero "; string_of_int x]
  | Zero x -> String.concat "" ["Zero "; string_of_int x]
  | Jump x -> String.concat "" ["Jump "; string_of_int x]
  | JumpMemLoc x -> String.concat "" ["JumpMemLoc "; string_of_int x]
  | Assign (x, y) -> String.concat "" ["Assign ("; string_of_int x; ", "; string_of_int y; ")"]
  | PushToStack x -> String.concat "" ["PushToStack "; string_of_int x]
  | AssignFromStack (x, y) -> String.concat "" ["AssignFromStack ("; string_of_int x; ", "; string_of_int y; ")"]
  | PushE x -> String.concat "" ["PushE "; string_of_int x]
  | PopE -> "PopE"
  | PushToEnv x -> String.concat "" ["PushToEnv "; string_of_int x]
  | AssignFromEnv (x, y) -> String.concat "" ["AssignFromEnv ("; string_of_int x; ", "; string_of_int y; ")"]
  | UpdateToEnv (x, y) -> String.concat "" ["UpdateToEnv ("; string_of_int x; ", "; string_of_int y; ")"]
  | Lock _ -> "Lock - UNSUPPORTED"
  | Unlock _ -> "Unlock - UNSUPPORTED"
  | Thread _ -> "Thread - UNSUPPORTED"
  )

let rec print_instr_list_helper lst num = (match lst with
  | [] -> ""
  | i::is -> let rest = (print_instr_list_helper is (num + 1)) in
             String.concat "" [string_of_int num; ": "; (print_instruction i); "\n"; rest]
  )

let print_instr_list instrs = print_instr_list_helper instrs 0
...
let singleStep st id mem memLock t = match (List.nth t.code !(t.pc)) with
  ...
  | Skip ->   inc t.pc;
              false
  | Label l -> raise_symb_label_in_vm_code l
  ...
  | NonZero i -> let x = t.stack.(!(t.sp) - 1) in
                 ...
                 (* (if x =0
                  then print_string ""
                  else Printf.printf "NonZero FROM %d TO %d\n" !(t.pc) i); *)
                 (if x == 0
                 then inc t.pc
                 else t.pc := i);
                 false
  ...
  | Zero i ->    let x = t.stack.(!(t.sp) - 1) in
                 dec t.sp;
                 (* (if x = 0
                  then Printf.printf "NonZero FROM %d TO %d\n" !(t.pc) i
                  else print_string ""); *)
                 (if x == 0
                 then t.pc := i
                 else inc t.pc);
  ...
  | Jump i ->
              (* Printf.printf "JUMP FROM %d " !(t.pc); *)
              t.pc := i;
              (* Printf.printf "TO %d\n" !(t.pc); *)
              false
  ...
  | PushE i -> t.env.(!(t.ep)) <- i;
               (* Printf.printf "PushE %d to pos %d - PC is %d\n" i !(t.ep) !(t.pc); *)
               inc t.pc;
               inc t.ep;
               false
  ...
  | PushToEnv loc -> inc t.pc;
                     (* Printf.printf "PushToEnv %d - PC is %d\n" mem.(loc) (!(t.pc) - 1); *)
                     t.env.(!(t.ep)) <- mem.(loc);
                     inc t.ep;
                     false
  ...
  | AssignFromEnv (relPos,loc) -> inc t.pc;
                                  ...
                                  (* Printf.printf "AFE - %d from %d - PC is %d\n"  mem.(loc) relPos (!(t.pc) - 1); *)
                                  false

  | UpdateToEnv (relPos,loc) -> inc t.pc;
                                t.env.(!(t.ep) - relPos) <- mem.(loc);
                                (* Printf.printf "UTE - %d  to %d  - PC is %d\n"  mem.(loc) relPos (!(t.pc) - 1); *)
                                false
